...
if ($region -notin $available_regions) {
    Write-Error "Invalid region: $region. Please select a valid AWS region."    
    exit 1
}

if ($polly_voice_id -notin $available_voice_ids) {
    Write-Error "Invalid voice id: $polly_voice_id. Please select a valid voice id."    
    exit 1
}


if ($polly_language_code -notin $available_language_codes) {
    Write-Error "Invalid language code: $polly_language_code. Please select a valid language code."    
    exit 1
}
